...
;**************************************************************
	option	casemap:none
	.386P
;	.model	flat,syscall,os_os2		am:cause syntax error
	.model	flat,syscall
...
;���������������
	...
	mov	BPTR [esi-1],'\'
	call	shim_i21_getcwd		;INT21 47h → shim
...
;	mov	scrnbufseg,ax
	...
	mov	vmodeold,13h		;INT 10h AH=0Fh replaced — dummy 0x13
...
exit::
	...
	call	shim_i21_setcd		;INT21 3bh → shim
...
viddat	db	30 dup (?)
 ...
 SUBR	vid_setvmode
	;-- Replaced: entire VGA mode-set sequence → SDL2 init shim --
	call	_shim_vid_init
...
Y=5
 ...
 SUBRP	help_main
	...
	I21OPENR				;shim: open file → EAX=handle (CF=error)
	jc	xx
	...
	mov	ecx,80*50			;# bytes
	mov	edx,offset buf
	I21READ					;shim: read file (BX=handle,ECX=count,EDX=buf) → EAX=bytes
	jc	x				;Error?
...
nodrv:
	...
	;-- Replaced: INT 21h 4Eh Find 1st dir → shim --
	mov	cx,10h				;Dir bit
...
dnext:	;-- Replaced: INT 21h 4Fh Find next dir → shim --
...
nodir:
	push	edx
	;-- Replaced: INT 21h 4Eh Find 1st file → shim --
	CLR	ecx				;Only files
...
set:	call	dir_insert
	...
	;-- Replaced: INT 21h 4Fh Find next file → shim --
	call	shim_i21_findnext
	jnc	sclp
...
;* Trashes none
 ...
 SUBRP	waiton_keyormouse
	;-- Replaced: INT 33h/16h → SDL2 shim --
	pushad
...
lp:	push	ecx
	...
	mov	cl,bh
	and	cl,3
	;-- Replaced: SC_MAPMASK → shadow plane ptr --
	movzx	edx,cl
...
lpx:	push	ecx
	...
	mov	cl,bl
	and	cl,3
	;-- Replaced: SC_MAPMASK + GC_MAPSEL → shadow plane ptr --
	movzx	edx,cl
	mov	edx,[_g_plane_ptrs + edx*4]
...
RNG=32000
 ...
 SUBR	gad_mousescroller
	...
	mov	mscrollcode_p,eax
					;Check shift keys
	;-- Replaced: BIOS 0:417h shift-key byte → SDL shim --
	push	ecx			; preserve — __cdecl clobbers ECX/EDX
	push	edx
...
noshft:
	...
	;-- Replaced: INT 33h 7/8/4 range+move → capture SDL drag anchor --
...
strt:	push	edi
	...
	pop	edi
nomv:	;-- Replaced: INT 33h 3 Read mouse → anchor-relative scroller shim --
...
lpx:	push	ecx
	...
	sub	DPTR [esp],3
	jmp	setup
do1:	;-- Replaced: SC_MAPMASK → shadow plane ptr --
	movzx	edx,cl
	mov	edx,[_g_plane_ptrs + edx*4]
setup:
	mov	cx,6[esi]		;Y size
...
lpx:	push	ecx
	...
	add	di,bp			;+Y offset
	and	cl,3
	;-- Replaced: SC_MAPMASK → shadow plane ptr in ebp (bp = Y offset saved/restored) --
	movzx	eax,cl

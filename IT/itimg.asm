...
;-- Export image/palette globals for C access (ImGui overlay) --
	...
	public	scrseqmem_p, scrseqbytes, ilpalloaded
...
;���������������
	...
	;-- Replaced: BIOS shift status (ds:[417h]) → SDL keyboard state --
	push	edx			; preserve DH (key scan) — __cdecl clobbers EDX
...
;	add	di,tw4			;+Y
	and	cl,3
	;-- Replaced: SC_MAPMASK → shadow plane ptr in ebx --
	movzx	ebx,cl
...
;* Clear img window
 ...
 SUBRP	iwin_clr
	;-- Replaced: all-plane clear of 320x200 image window → C shim --
	call	_shim_iwin_clr
	ret
...
;	call	crossh_draw
	...
	jz	noimg			;None?
	mov	ebx,eax
	mov	tl1,ebx			; save IMG* — draw loop clobbers EBX with plane ptr
...
lpx:	push	ecx
	...
	add	di,tw4			;+Y
	and	cl,3
	;-- Replaced: SC_MAPMASK → shadow plane ptr cached in _vga_base_p --
	movzx	eax,cl

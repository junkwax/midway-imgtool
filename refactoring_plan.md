...
Branch: `refactor/overlay-split` (67 commits ahead of `SDL-main`). Working tree
clean. `imgui_overlay.cpp` is down to 18,982 lines; `ui_canvas.cpp` is now 1,855
...
preview). `ui_canvas` has started with zoom/pan helpers, the single-sprite
...
- [ ] `ui_canvas` — canvas render, pan/zoom, World View. **Started:** the
      zoom/pan helpers, single-sprite World View canvas, World View config/layout state, onion-skin

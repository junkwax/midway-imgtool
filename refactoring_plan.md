...
Branch: `refactor/overlay-split` (48 commits ahead of `SDL-main`). Working tree
clean. `imgui_overlay.cpp` is down to 19,792 lines; `ui_canvas.cpp` is now 801
...
source/dummy-decap lane builders. Supporting
...
| `ui_canvas.{h,cpp}` | single-sprite World View canvas, World View config state, onion texture cache, marked World View lane model/constants/playback+sequence+mirror+dummy-decap+drag+ASM-popup state/string/model helpers, marked-frame collection, source/dummy-decap lane builders | — |
...
- [ ] `ui_canvas` — canvas render, pan/zoom, World View. **Started:** the
      ...
      source/dummy-decap lane builders now live in `ui_canvas`; marked-tab
      drawing and the regular edit canvas remain in `imgui_overlay.cpp`.

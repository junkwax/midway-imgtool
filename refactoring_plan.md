...
Branch: `refactor/overlay-split` (63 commits ahead of `SDL-main`). Working tree
clean. `imgui_overlay.cpp` is down to 19,149 lines; `ui_canvas.cpp` is now 1,636
...
marked World View scene drawing, marked-lane panel layout/header controls, per-lane edit controls, thumbnail strip, drag handling, sprite/tag/status drawing, and
...
| `ui_canvas.{h,cpp}` | single-sprite World View canvas, World View config/layout state, onion texture cache, marked World View scene drawing, marked World View lane model/constants/playback+sequence+mirror+dummy-decap+drag+ASM-popup state/string/model helpers, marked-frame collection, source/dummy-decap/ASM lane builders, lane playback resolver/sequence refresh, selected dummy-decap assignment, panel layout/header controls, per-lane edit controls, thumbnail strip, drag, sprite/tag/status drawing, render rects + ASM generation/preview | — |
...
- [ ] `ui_canvas` — canvas render, pan/zoom, World View. **Started:** the
      ...
      strip, marked scene drawing, and sprite panel layout/header controls/drag handling, tag/status
      drawing, render rect bookkeeping, and ASM export text generation/preview
      now live in `ui_canvas`; the regular edit canvas

...
Branch: `refactor/overlay-split` (42 commits ahead of `SDL-main`). Working tree
clean. `imgui_overlay.cpp` is down to 19,997 lines; `ui_canvas.cpp` is now 595
...
onion-skin texture cache, and marked World View constants, playback/sequence/
mirror/dummy-decap/drag state, string/model helpers, sequence tick/sync/edit
helpers, and dummy-decap timing reset helpers. Supporting
...
| `ui_canvas.{h,cpp}` | single-sprite World View canvas, onion texture cache, marked World View constants/playback+sequence+mirror+dummy-decap+drag state/string/model helpers | — |
...
- [ ] `ui_canvas` — canvas render, pan/zoom, World View. **Started:** the
      single-sprite World View canvas, onion-skin texture cache, marked World
      View constants, playback/sequence/mirror/dummy-decap/drag state,
      string/model helpers, sequence tick/sync/edit helpers, and dummy-decap
      timing reset helpers now live in `ui_canvas`; marked-tab drawing,
      dummy-decap detection, ASM export popup state, and the regular edit
      canvas remain in `imgui_overlay.cpp`.

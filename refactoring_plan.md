...
Branch: `refactor/overlay-split` (64 commits ahead of `SDL-main`). Working tree
clean. `imgui_overlay.cpp` is down to 19,138 lines; `ui_canvas.cpp` is now 1,659
...
| `ui_canvas.{h,cpp}` | single-sprite World View canvas, World View config/layout state, onion texture cache, marked World View scene drawing, marked World View lane model/constants/playback+sequence+mirror+dummy-decap+drag+ASM-popup state/string/model helpers, marked-frame collection, source/dummy-decap lane collection/builders, ASM lane builder, lane playback resolver/sequence refresh, selected dummy-decap assignment, panel layout/header controls, per-lane edit controls, thumbnail strip, drag, sprite/tag/status drawing, render rects + ASM generation/preview | — |
